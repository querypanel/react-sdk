...
export function LoginForm({
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      trackEvent("Sign In", { method: "email-password" });
      identifyUser(email, { email, sign_in_method: "email-password" });
  ...
  const signInWithGithub = async () => {
    ...
    try {
      ...
      trackEvent("Sign In", { method: "github" });
    } catch (error: unknown) {
      setError(error instanceof Error ? error.message : "An error occurred");
    ...
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <Card className="border-0 shadow-xl bg-background/80 backdrop-blur-sm">
        <CardHeader className="space-y-1">
          <CardTitle className="text-xl text-center">Sign in to QueryPanel</CardTitle>
          <CardDescription className="text-center">
            Sign in to continue creating amazing visualizations
          </CardDescription>
        </CardHeader>
        <CardContent>
